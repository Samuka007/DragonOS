...
pub struct RamFSInode {
    ...
    /// 子Inode的B树
    children: BTreeMap<Arc<String>, Arc<LockedRamFSInode>>,
    /// inode_id index
    id_index: BTreeMap<InodeId, Weak<String>>,
    /// 当前inode的数据部分
    data: Vec<u8>,
    /// 当前inode的元数据
    metadata: Metadata,
...
impl IndexNode for LockedRamFSInode {
    ...
    fn create_with_data(
        ...
        let name_ref = Arc::new(String::from(name));

        // 将子inode插入父inode的B树中
        inode.children.insert(name_ref.clone(), result.clone());

        // 将inode插入inode索引中
        inode.id_index.insert(id_tmp, Arc::downgrade(&name_ref));
    ...
    fn link(&self, name: &str, other: &Arc<dyn IndexNode>) -> Result<(), SystemError> {
        ...
        inode
            .children
            .insert(name_ref.clone(), other_locked.self_ref.upgrade().unwrap());

        inode
            .id_index
            .insert(other_locked.metadata.inode_id, Arc::downgrade(&name_ref));
    ...
    fn get_entry_name(&self, ino: InodeId) -> Result<String, SystemError> {
        ...
        match ino.into() {
            ...
            _ => {
                ...
                match key {
                    Some(k) => {
                        return Ok(k.upgrade().unwrap().to_string());
                    }
                    None => {
    ...
    fn list(&self) -> Result<Vec<String>, SystemError> {
        ...
        keys.push(String::from(".."));
        keys.extend(self.0.lock().children.keys().map(|k| k.to_string()));
    ...
    fn mknod(
        ...
        let fname_ref = Arc::new(String::from(filename).to_uppercase());

        inode
            .children
            .insert(fname_ref.clone(), nod.clone());

        inode
            .id_index
            .insert(nod.0.lock().metadata.inode_id, Arc::downgrade(&fname_ref));
        Ok(nod)
    }
